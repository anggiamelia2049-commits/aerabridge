<form action="{{ route('user-edukasi-progress.store') }}" method="POST">
    {{ csrf_field() }}
    ...
    <select name="konten_id">
        ...
        <option value="{{ $item->id }}" {{ old('konten_id') == $item->id ? 'selected' : '' }}>
            {{ $item->judul }}
        </option>
        @endforeach
    </select>
    ...
    Progress (%) :
    <input type="number" name="progress" min="0" max="100" value="{{ old('progress') }}">
    ...
    Selesai Pada :
    <input type="date" name="selesai_pada" value="{{ old('selesai_pada') }}">
