<form action="{{ route('super_admin.deteksi-ai.store') }}" method="POST">
    {{ csrf_field() }}
    ...
    Jenis Objek :
    <input type="text" name="jenis_objek" value="{{ old('jenis_objek') }}">
    ...
    Confidence :
    <input type="number" step="0.01" min="0" max="1" name="confidence" value="{{ old('confidence') }}">
    ...
    <select name="estimasi_prioritas">
        ...
        <option value="kritis"
            {{ old('estimasi_prioritas') == 'kritis' ? 'selected' : '' }}>
            Kritis
        </option>

        <option value="sedang"
            {{ old('estimasi_prioritas') == 'sedang' ? 'selected' : '' }}>
            Sedang
        </option>

        <option value="rendah"
            {{ old('estimasi_prioritas') == 'rendah' ? 'selected' : '' }}>
            Rendah
        </option>
    </select>
    ...
    <select name="hasil_validasi">
        ...
        <option value="valid"
            {{ old('hasil_validasi') == 'valid' ? 'selected' : '' }}>
            Valid
        </option>

        <option value="tidak_valid"
            {{ old('hasil_validasi') == 'tidak_valid' ? 'selected' : '' }}>
            Tidak Valid
        </option>
    </select>
    ...
    Response LLM :
    <textarea name="response_llm">{{ old('response_llm') }}</textarea>
